...
Чтобы в режиме Яндекс.Диска видеть не весь диск, а только определённую папку:

```env
ALEXANDRITE_YANDEX_ROOT_PATH=<yandex-root-directory>
```

Если переменная не задана или пустая — доступен весь Яндекс.Диск. Папка для синхронизации и архивов задаётся отдельно через `YANDEX_DISK_ALEXANDRITE_PATH`. Локальный режим использует отдельную папку `ALEXANDRITE_VAULT_PATH`.
