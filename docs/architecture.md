...
- **Режим Яндекс.Диска** — read-only просмотр Markdown-файлов и папок. Область просмотра ограничивается через `ALEXANDRITE_YANDEX_ROOT_PATH` (пустое значение — весь диск), а папка синхронизации и архивов задаётся через `YANDEX_DISK_ALEXANDRITE_PATH`; локальный режим использует отдельный `ALEXANDRITE_VAULT_PATH`.
