...
- **Канбан** — доски задач внутри проектов и общая доска: колонки, приоритеты, ответственные, дедлайны, вложения, закрытие задач. Поддерживается drag-and-drop и прямые ссылки на задачи.
- **Список задач (ToDo)** — таблица всех задач с фильтрами и сортировкой, импорт задач из ToDo-списков, выгрузка в `.txt`/JSON/CSV.
