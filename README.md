...
📖 Документация проекта собирается MkDocs в каталог `site/` и может быть опубликована на выбранном статическом хостинге.
...
- **Дела и ежедневная сводка** — личные и общие заметки, дедлайны, привязка к проектам и информационная лента.
- **Комментарии проектов** — обсуждения с отметкой прочтения и счётчиками новых сообщений.
- **MCP для AI-ассистентов** — защищённая работа с проектами и задачами через HTTP MCP.
- **Подключаемые приложения** — развёртывание готовых Docker-образов через форму с журналом установки, health-check и включением в меню; в Compose Docker-операции изолированы во внутреннем `extension-manager`.
...
Приложение доступно по адресу: `http://<armory-host>:<armory-port>`.
...
После запуска `mkdocs serve` документация доступна по адресу, указанному в параметре `--dev-addr`.
